...
wire  [127:0]  outdata,key       ; //UDP待发送数据
wire  [127:0]  read_data,textout,test,des_aes_out,test1;//UDP待发送数据
wire [2:0]test2;
wire [4:0] test3      ; 
wire[6:0]test4 ;
wire          outready,des_aes_ready   ; //UDP发送完成信号
wire          outsend   ; //UDP发送完成信号
wire    read_req,ide;
wire [3:0] cntt;
//*****************************************************
//**                    main code
...
assign tx_start_en = outready;//接收包数据完成后，给出来的完成信号，给到发送信号
assign tx_byte_num = rec_byte_num-6'd16; //fa发送字节数量
...
wire start;
//同步FIFO
fifo_generator_0 u_dasdasync_fifo_2048x32b (
...
read_and_out mmmmm(
    ...
    .read_data (read_data   ),  // output wire [31 : 0] dout
...
sync_fifo_2048x32b u_sync_fifo_2048x32b (
    .clk      (gmii_rx_clk),  // input wire clk
    .rst      (~sys_rst_n),  // input wire rst
    .din      (des_aes_out  ),  // input wire [31 : 0] din
