...
DESkey   key(.in(key_in),.C0(i[1:28]),.D0(i[29:56]));//进行置换选择1，并且把密钥分为左右两部分

left_shiftera   left_shiftera1 (.in(i[1:28]),.outM(k1_c)),  //按照规则左移1位
...
left_shifterb   left_shifterb1(.in(k2_c),.outN(k3_c)),  //按照规则左移2位
...
DESkeys       keys1(.C(k1_c),.D(k1_d),.out(key1)),  //置换选择2
