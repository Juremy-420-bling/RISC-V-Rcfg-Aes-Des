module des_aes_top(
    ...
    input           [127:0]     in,         //DES_AES模块接受数据[127：0]
    input           [127:0]     key,        //DES_AES模块接受密钥
    output   reg                ready,      //DES_AES模块向fifo发送加解密使能
    output   reg    [127:0]     out,        //DES_AES模块向fifo发送加解密结果
    output                      outready,   //DES_AES模块向以太网发送读使能
    output   reg                ide         //DES_AES模块空闲信号
);  
    reg[2:0]        count;
    ...
    AESkeyprocess FINALKEY(key,outaeskey);                          //产生所有AES的密钥
  ...
  assign desfikey1=endeskey(cnt1);      //用cnt1对应desfikey、aesfikey 
  assign aesfikey1=enaeskey(cnt1);
  assign desfikey2=endeskey(cnt2);      //用cnt2对应desfikey、aesfikey
  assign aesfikey2=enaeskey(cnt2);
  assign desfikey3=endeskey(cnt3);      //用cnt3对应desfikey、aesfikey  
  assign aesfikey3=enaeskey(cnt3);
  assign desfikey4=endeskey(cnt4);      //用cnt4对应desfikey、aesfikey
  assign aesfikey4=enaeskey(cnt4);
    ...
    always @ (posedge clk)begin
        ...
        if(go) begin                    //判断如果mode为AES加密,这里只进行一次性加密
            if(cnt0 <pick) begin        //小于pick，所以cnt0最终会停留在pick
                cnt0 <= cnt0 + 5'b00001;
            end
                                        //根据cnt0、选择不同的流水线工作状态
            if(cnt0<=5'd3)begin
           ...
           if(cnt0==5'd16)begin
            ...
            if(count==3'd5&&countall<7'd10)begin        //countall代表desaes的加解密次数
                ide<=1;
            end
            end
    ...
    /*init process*/
    init_process process(MODE,text0,key,endaeskey_tmp,out0);  //这一级是数据的初始化，直接得到第一步开始所需要的内容以及密钥，也采用了流水线
